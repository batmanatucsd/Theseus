...
class priority_queue : public std::priority_queue<T, Container, Compare> {
  public:
    ...
    void remove(const T& val) {
      iterator nodeToRemove = find(val); // find the node to remove
      // set the F value of the node
      //to bring to the top of the queue 
      if(!this->empty()) {
      maze[*nodeToRemove].setF(0);
      this->pop(); // remove the node
      }
    }
};
...
void findShortestPath() {
  ...
  //while(!queue.empty()) {
  while(!queue.empty() && maze[goal].getF() <= maze[queue.top()].getF()) {
    current = queue.top();
    ...
    for(byte next=0; next<4; ++next) {
      ...
      neighbor = current + neighbors[next]; // get neighbor 
      neighborNode = &maze[neighbor]; // get neighbor node as pointer
      ...
      if(neighbor == goal) {
        ...
        //if(neighborNode->getF() < maze[queue.top()].getF())
          return;
      }
      ...
      // remove from the open queue if new g value is better
      if(isInOpen && newG < neighborNode->getG())
        queue.remove(neighbor);

      // remove from closed queue if new g value is better
      if(neighborNode->isVisited() && newG < neighborNode->getG())
        neighborNode->unVisit();

    ///////// DEBUG MESSAGE ///////////////
    std::cout << "             neighbor: " << (int) neighbor; 

      // if the neighbor is not in open OR 
      // the new g value is smaller than the old g value
      //if(!isInOpen || newG < currentNode->getG()) {
      if(!isInOpen && !neighborNode->isVisited()) {
